...
function Shop() {
	...
	function Submit() {
		if (
			parseInt(document.querySelector("#amount").value) >=
			featureContext.currentFeature.currentBid + 100
		) {
			console.log("wut");
			setIsError(false);
			featureContext.raiseBid(
				parseInt(document.querySelector("#amount").value)
			);
			...
			setErrorMessage(
				"Your bid must be more than the current bid by at least $100"
			);
			setIsError(true);
		}
	}
	...
	return (
		<>
			<main className="shop-main">
					...
					>
						<GridItem className="feature-header" colSpan={5}>
							...
							<h2 className="feature-artist">
								{featureContext.currentFeature.authorFirstName +
									" " +
									featureContext.currentFeature
										.authorLastName}
							</h2>
							<GridItem className="feature-name">
								...
								<p className="feature-year">
									(
									{featureContext.currentFeature.year ||
										"c. '05"}
									)
								</p>
							</GridItem>
						...
						<GridItem className="feature-about">
							<p className="para">
								<u>About the feature from the artist:</u>
							</p>
							<br></br>
							<p className="para2">
						...
						<GridItem className="bid-info">
							<h4 className="bid-start">
								The starting bid{" "}
								{featureContext.currentFeature.startPrice !==
								featureContext.currentFeature.currentBid
									? "was"
									: "is"}
								:{" "}
								<u>
				...
				<button
					onClick={rotateFeatures}
					className="hiddenAuctionButton"
				>
					{" "}
					Dev: Progress Feature Queue{" "}
				</button>
			</main>
		</>
	);
